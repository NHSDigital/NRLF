Feature: Consumer - searchDocumentReference - Failure Scenarios
  ...
  Scenario: Search fails to return a bundle when extra keys are found
    ...
    When the organisation 'RX898' requests to search for DocumentReferences using POST with request body:
      | key     | value      |
      | subject | 9278693472 |
      | extra   | parameter  |
  ...
  Scenario: Search rejects request with type system they are not allowed to use
    ...
    And the OperationOutcome contains the following issue:
      """
      {
        ...
        "diagnostics": "Invalid type (The provided type system does not match the allowed types for this application)",
        "expression": ["type"]
      }
      """
  ...
  Scenario: Search rejects request if the organisation has no registered pointer types
    ...
    And the OperationOutcome contains the following issue:
      """
      {
        ...
        "diagnostics": "Your organisation does not have permission to access this resource. Contact the onboarding team."
      }
      """
  ...
  Scenario: Search ignores pointer type header if S3 lookup is enabled
    ...
    And a DocumentReference resource exists with values:
      | property    | value                           |
      | id          | 02V-1114567890-SearchDocRefTest |
      | subject     | 9278693472                      |
      | status      | current                         |
      | type        | 736253002                       |

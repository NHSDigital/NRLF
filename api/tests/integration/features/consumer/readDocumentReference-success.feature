...
Feature: Consumer - readDocumentReference - Success Scenarios
  ...
  Scenario: Read document reference by ID - urlencoded
    ...
    And a DocumentReference resource exists with values:
      | property    | value                                      |
      | id          | RX898\|001-1114567890-ReadDocRefUrlEncoded |
      | subject     | 9278693472                                 |
      | status      | current                                    |
      | type        | 736253002                                  |
      | contentType | application/pdf                            |
      | url         | https://example.org/my-doc.pdf             |
      | custodian   | RX898                                      |
    When the organisation 'RX898' requests to read a DocumentReference with ID 'RX898%7C001-1114567890-ReadDocRefUrlEncoded'
    Then the response status code is 200

...
data "external" "raise-schema-error" {
  count   = jsondecode(data.aws_lambda_invocation.rds-cluster-schema-creation.result)["status"] == "ERROR"
  program = ["python", "-c", "print(${data.aws_lambda_invocation.rds-cluster-schema-creation.result}); raise Exception;"]

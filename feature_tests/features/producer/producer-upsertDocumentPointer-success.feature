Feature: Producer Create Success scenarios
  ...
  Scenario: Successfully upsert a Document Pointer of type Mental health crisis plan
    Given Producer "Aaron Court Mental Health NH" (Organisation ID "8FW23") is requesting to upsert Document Pointers
    And Producer "Aaron Court Mental Health NH" is registered in the system for application "DataShare" (ID "z00z-y11y-x22x") with pointer types
      | system                 | value     |
      | http://snomed.info/sct | 736253002 |
    When Producer "Aaron Court Mental Health NH" upserts a Document Reference from DOCUMENT template
      | property    | value                          |
  ...
  Scenario: Successfully upsert a Document Pointer of type End of life care coordination summary
    Given Producer "Aaron Court Mental Health NH" (Organisation ID "8FW23") is requesting to upsert Document Pointers
    And Producer "Aaron Court Mental Health NH" is registered in the system for application "DataShare" (ID "z00z-y11y-x22x") with pointer types
      | system                 | value           |
  ...
  Scenario: Successfully upsert a Document Pointer with content and contact details
    Given Producer "Aaron Court Mental Health NH" (Organisation ID "8FW23") is requesting to upsert Document Pointers
    And Producer "Aaron Court Mental Health NH" is registered in the system for application "DataShare" (ID "z00z-y11y-x22x") with pointer types
      | system                 | value     |
      | http://snomed.info/sct | 736253002 |
    When Producer "Aaron Court Mental Health NH" upserts a Document Reference from DOCUMENT_WITH_CONTACT template
      | property    | value                               |
      | identifier  | 1234567890                          |
      | type        | 736253002                           |
      | custodian   | 8FW23                               |
  ...
  Scenario: Successfully upsert a Document Pointer with contact details only
    Given Producer "Aaron Court Mental Health NH" (Organisation ID "8FW23") is requesting to upsert Document Pointers
    And Producer "Aaron Court Mental Health NH" is registered in the system for application "DataShare" (ID "z00z-y11y-x22x") with pointer types
      | system                 | value     |
      | http://snomed.info/sct | 736253002 |
    When Producer "Aaron Court Mental Health NH" upserts a Document Reference from DOCUMENT_WITH_CONTACT_ONLY template
      | property   | value                               |
      | identifier | 1234567890                          |
      | type       | 736253002                           |
      | custodian  | 8FW23                               |
  ...
  Scenario: Successfully upsert a Document Pointer when the producer has an extension code
    Given Producer "BaRS (EMIS)" (Organisation ID "V4T0L.YGMMC") is requesting to upsert Document Pointers
    And Producer "BaRS (EMIS)" is registered in the system for application "DataShare" (ID "z00z-y11y-x22x") with pointer types
      | system                 | value           |
      | http://snomed.info/sct | 861421000000109 |
    When Producer "BaRS (EMIS)" upserts a Document Reference from DOCUMENT template
      | property    | value                          |
      | identifier  | 1234567891                     |
      | type        | 861421000000109                |
  ...
  Scenario: Successfully upsert a Document Pointer with overridden created on date when producer has permission to set audit date
  ...
  Scenario: Validate a Document Pointer of type TEST_TYPE with good URL
    ...
    When Producer "Aaron Court Mental Health NH" upserts a Document Reference from DOCUMENT_WITH_DATE template
      | property    | value                          |
      | identifier  | 1234567890                     |
      | type        | TEST_TYPE                      |
      | custodian   | 8FW23                          |
  ...
  Scenario: Validate a Document Pointer of type Mental health crisis plan using the asid data contract with no ssp and asid
    ...
    And Producer "Aaron Court Mental Health NH" is registered in the system for application "DataShare" (ID "z00z-y11y-x22x") with pointer types
      ...
      | http://snomed.info/sct | 736253002 |
    And the Data Contracts are loaded from the database
    When Producer "Aaron Court Mental Health NH" upserts a Document Reference from DOCUMENT_WITH_AUTHOR template
      | property    | value                                    |
      | identifier  | 1234567890                               |
      | type        | 736253002                                |
      | custodian   | 8FW23                                    |
